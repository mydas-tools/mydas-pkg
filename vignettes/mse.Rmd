---
title: "Management Strategy Evaluation **FLife**"
subtitle: "Data Rich"
...
set.seed(1234)
srDev=FLife:::rlnoise(nits,fbar(om)%=%0,.3,b=0.0)
```
...
oms=FLStocks("Projection"=fwd(om,fbar=fbar(om)[,-1],residuals=srDev,sr=eql))
```

```{r stock-stochastic-2, echo=FALSE}
plot(oms[["Projection"]])+
  geom_line(aes(year,data,col=iter,group=iter),
            data=as.data.frame(FLQuants(iter(oms[["Projection"]],c(7,12,19)),
...
```{r emp-mse-plot, echo=FALSE,eval=FALSE}
