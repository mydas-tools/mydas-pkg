...
mseSBTP<-function(
  ...
  maxF=median(FLQuant(1,dimnames=dimnames(sr_deviances))%*%apply(fbar(window(om,end=start)),6,max)*maxF)
  ...
  for (iYr in seq(start,end,interval)){
    ...
    cpue[,ac(iYr-(interval:1))]=ssb(om)[,ac(iYr-(interval:1))]%*%u_deviances[,ac(iYr-(interval:1))]
